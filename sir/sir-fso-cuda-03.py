...
Usage:
python sir-fso-cuda-02.py -p <inputpath> -i <inputfile> -d<debug> --sx <num1> --sy <num2> --ex <num3> --ey <num4> -v <videofile> -o<output>
Example:
Python sir-fso-cuda-02.py -p d:\data\  -i *.fits -d True --sx 250 --sy 250 --ex 750 --ey 750 -v test.avi -o True
...
def main(argv):
	...
	for opt, arg in opts:
		if opt == '-h':
			...
			print ('Usage: python sir-fso-cuda-02.py -p <inputpath> -i <inputfile> --sx <num1> --sy <num2> --ex <num3> --ey <num4> -v <videofile> -o')
			print ('Example: python sir-fso-cuda-02.py -p d:\\fso-test -i *.fits -d False --sx 200 --sy 200 --ex 700 --ey 700  -v test.avi -o True')
			sys.exit()
...
#            plt.show()
				...
				dt = end - start
				dt1 = int(dt.total_seconds() * 1000)
				total_time = total_time + dt1
				pstr = ('id:%5d   dx=%7.2f   dy=%7.2f   corr=%7.2f Runtime=%d ms  %s' % (
				i, dis_s_subpix[0], dis_s_subpix[1], cor, dt1, filename[i][dirnlen:]))
