...
if [ $# -ne 8 ];then
  ...
  echo "Example: ./data-sum-missing.sh  /lustre/data 2020 0928  2020 1001 TIO 1 1"
  echo "         ./data-sum-missing.sh  /lustre/data 2020 0928  2020 1001 HA 0 0"
  exit 1
fi
...
do
    ...
    if [ ! -f $homepre/$checkyear/$datatype-$checkyear-$checkmonthday.sum ];then
        ...
        if [ $report -eq "1" ];then 
            echo $homepre/$checkyear/$datatype-$checkyear-$checkmonthday.sum >>  $suminfo
        fi
	#    echo "$i $checkdate"
    fi
    let i++
