...
remoteerrlist=$logpath/$datatype-$fileformat-$today-diff.list
# local wrongsize list
errlist=$logpath/$datatype-localwrongsize.list
targetdir=/lustre/data/$year/$year$monthday
...
if [ $errsize4 -eq 0 ]; then
  echo "$today $ctime3: $datatype data under /lustre/data/$year/$year$monthday/$datatype are O.K.!" | mail -s "lustre-$datatype@$year$monthday: $errsize4 Error File(s) Found" nvst_obs@ynao.ac.cn
  echo "$today $ctime3: $datatype data under /lustre/data/$year/$year$monthday/$datatype are O.K.!" | mail -s "lustre-$datatype@$year$monthday: $errsize4 Error File(s) Found" chd@ynao.ac.cn
else
  mail -s "lustre-$datatype@$year$monthday: $errsize4 Error File(s) Found" nvst_obs@ynao.ac.cn < $tmplist
  mail -s "lustre-$datatype@$year$monthday: $errsize4 Error File(s) Found" chd@ynao.ac.cn < $tmplist
fi
