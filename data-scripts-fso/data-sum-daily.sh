...
#start=`find ./ -name "*.fits" -type f ! -name "FLAT*" -type f ! -name "Dark*" -type f ! -name "redata*" |stat *|grep Change|awk '{print $2 " " $3}'| sort |head -n +1`
#end=`find ./ -name "*.fits" -type f ! -name "FLAT*" -type f ! -name "Dark*" -type f ! -name "redata*" | stat *|grep Change|awk '{print $2 " " $3}'| sort |tail -n -1`
start=`find ./ -name $dataprefix*.fits -type f | stat *|grep Change|awk '{print $2 " " $3}'| sort |head -n +1`
end=`find ./ -name $dataprefix*.fits -type f | stat *|grep Change|awk '{print $2 " " $3}'| sort |tail -n -1`
#touch filetime-00
