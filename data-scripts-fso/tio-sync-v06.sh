...
echo "                  tio-sync.sh                          "
echo "          (Release 0.6 20190604 19:42)                 "
echo "                                                       "
echo "                $today $ctime                          "
echo "======================================================="
echo " "
...
if [ $procCmd -le 0 ];then
  ...
  lftp -u $user,$password -e "mirror --ignore-time --no-perms --continue --exclude /\$RECYCLE.BIN/$  --parallel=33  / .; quit" $srcdir1 >/dev/null 2>&1 &
  waiting "$!" "Syncing..."
