...
do
  ...
  wget  -q --tries=3 --timestamping --retry-connrefused --timeout=10 --continue --inet4-only --ftp-user=$user --ftp-password=$password --no-host-directories --recursive  --level=0 --no-passive-ftp --no-glob --preserve-permissions $srcdir  >> $datatype.lock &
  itmp=$((itmp-1))
  #echo $itmp
  #sleep 0.5
done

while [[ $(cat $datatype.lock | wc -l) < $threadnumber ]]; do
  ctimet=`date --date='0 days ago' +%H:%M:%S`
  echo "$today $ctimet: Waiting for Syncing $datatype @ $srcdir1"
  sleep 1
done
