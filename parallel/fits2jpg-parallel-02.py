...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv, "hp:p:t:x:y:", [
                                   "help", "input_path=", "tn=", "sx=", "sy="])
    ...
    for opt, arg in opts:
        if opt == '-h':
            print(
                'Usage: python fits2jpg-xx.py -p <inputpath> --tn <num> --sx <num1> --sy <num2> ')
            print(
    ...
    delta = b - a
    print("Time Used with %d thread : %d ms" %
          (tn, int(delta.total_seconds() * 1000)))
